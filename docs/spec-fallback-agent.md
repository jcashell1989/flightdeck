# flightdeck — Agent Profiles & Managed Instance Design
...
flightdeck can launch and manage `opencode serve` processes on behalf of
...
via `⌘K`, flightdeck starts the appropriate `opencode serve` process if it
...
A running `opencode serve` process spawned by flightdeck:
...
flightdeck runs these commands internally. Shown here for transparency and
...
OPENCODE_MODEL=openrouter/kimi-k2.5 \
  opencode serve --port 4100   # flightdeck auto-assigns from 4100–4200
