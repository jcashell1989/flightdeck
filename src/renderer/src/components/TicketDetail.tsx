  ...
  return (
    <div style={{ padding: 16, height: '100%', overflowY: 'auto', fontSize: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <span style={{ background: 'var(--bg-element)', borderRadius: 4, padding: '2px 6px' }}>{ticket.status}</span>
        <span style={{ background: 'var(--bg-element)', borderRadius: 4, padding: '2px 6px' }}>{ticket.priority}</span>
        <span style={{ background: 'var(--bg-element)', borderRadius: 4, padding: '2px 6px' }}>{ticket.type}</span>
      </div>
      {ticket.description && (
        <div style={{ color: 'var(--fg)', marginBottom: 12, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>
          {ticket.description}
        </div>
      )}
        ...
        <div>
          ...
          {ticket.logs.slice().reverse().map((entry, i) => (
            <div key={i} style={{ marginBottom: 6, paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ color: 'var(--fg)', whiteSpace: 'pre-wrap' }}>{entry.message}</div>
            </div>
          ))}
        </div>
      )}
